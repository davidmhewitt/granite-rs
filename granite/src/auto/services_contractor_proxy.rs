...
impl<O: IsA<ServicesContractorProxy>> ServicesContractorProxyExt for O {
    fn connect_contracts_changed<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn contracts_changed_trampoline<P: IsA<ServicesContractorProxy>, F: Fn(&P) + 'static>(this: *mut ffi::GraniteServicesContractorProxy, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(ServicesContractorProxy::from_glib_borrow(this).unsafe_cast_ref())
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, b"contracts-changed\0".as_ptr() as *const _,
                Some(transmute::<_, unsafe extern "C" fn()>(contracts_changed_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }
}
