...
impl<O: IsA<ServicesSettingsSerializable>> ServicesSettingsSerializableExt for O {
    fn settings_serialize(&self) -> Option<glib::GString> {
        unsafe {
            from_glib_full(ffi::granite_services_settings_serializable_settings_serialize(self.as_ref().to_glib_none().0))
        }
    }

    fn settings_deserialize(&self, s: &str) {
        unsafe {
            ffi::granite_services_settings_serializable_settings_deserialize(self.as_ref().to_glib_none().0, s.to_glib_none().0);
        }
    }
}
